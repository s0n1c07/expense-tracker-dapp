...
contract ExpenseTracker {
    ...
    */
    ...
    function getOverdueDebts(address user) external view returns (
    address[] memory creditors,
    uint256[] memory amounts,
    uint256[] memory daysOld
) {
    uint256 count = 0;

    // First pass: count valid overdue debts
    for (uint256 i = 0; i < expenseCount; i++) {
        Expense storage e = expenses[i];
        if (e.amountOwed[user] > 0 && block.timestamp > e.timestamp + 2 days) {
            count++;
        }
    }
    ...
    for (uint256 i = 0; i < expenseCount; i++) {
        Expense storage e = expenses[i];
        if (e.amountOwed[user] > 0 && block.timestamp > e.timestamp + 2 days) {
            // find creditor (someone who paid in this expense)
            address creditor = address(0);
            for (uint256 j = 0; j < e.participants.length; j++) {
                address p = e.participants[j];
                if (p != user && e.amountPaid[p] > 0) {
                    creditor = p;
                    break;
                }
            }
            ...
            if (creditor != address(0)) {
                creditors[idx] = creditor;
                amounts[idx] = e.amountOwed[user];
                daysOld[idx] = (block.timestamp - e.timestamp) / 1 days;
                idx++;
            }
        }
    }

    return (creditors, amounts, daysOld);
}
